function updateSVG() {
    ...
    const addedHeight = screenHeight - 200; // Assuming 200 is the original height

    const svg = document.getElementById('my-svg');

    // Center the SVG within the viewport
    const xOffset = -screenWidth / 3.7; // Adjust as needed
    const yOffset = -screenHeight / 0.7 + addedHeight; // Adjust as needed

    // Set viewBox with the offset values
    svg.setAttribute('viewBox', `${xOffset} ${yOffset} ${screenWidth} ${screenHeight + addedHeight}`);

    // Set SVG width and height
    svg.setAttribute('width', screenWidth);
    svg.setAttribute('height', screenHeight + addedHeight);
